...
import IEmpSide from '@/layouts/components/IEmpSide/IEmpSide.vue'
...
const menuConfig = reactive({
  menuList: empRoutes, // 菜单列表
  menuListArr: convertToRouterArray(empRoutes), // 菜单列表数组
...
<template>
  <div class="i-layout">
    <a-layout style="flex-direction: column">
      <i-emp-header v-model:menu-collapsed="menuConfig.collapsed" v-model:open-drawer="menuConfig.openDrawer" />
      <a-layout style="gap: 1px">
        ...
        <a-layout class="i-layout__inner">
          ...
          <a-layout-footer>
            <div class="title">{{ $t('app.bottom') }} 2024 Created by pepedd</div>
